...
class leave extends StatefulWidget {
  ...
  const leave({super.key, required this.childId});
...
class _leaveState extends State<leave> {
  ...
  void LeaveApply() async {
    ...
    try {
      Response response = await post(
        Uri.parse('${ApiConfige.BASE_URL}/parent_apply_child_leave.php'),
        body: {
          ...
          showDialog(
            ...
            builder: (BuildContext context) {
              ...
              return AlertDialog(
                ...
                content: Text('Leave Applied', textAlign: TextAlign.center,),
  ...
  leavelistData() async {
    ...
    final response = await http.get(Uri.parse(
        '${ApiConfige.BASE_URL}/get_parent_child_leave_list.php?parent_id=${Constants.parentId}&child_id=${widget.childId}'));

    if (response.statusCode == 200) {
      final jsonData = json.decode(response.body);
      final leavedetails = jsonData['leave_list'];

      setState(() {
        leavedata = List<Map<String, dynamic>>.from(leavedetails);
      });
  ...
  //     builder: (BuildContext context) {
  //       return AlertDialog(
  //         title: Text('Delete applied leave date'),
  //         content: Text('Do you want to delete applied leave date ?'),
  //         actions: [
  //           TextButton(
  //             onPressed: () {
  ...
  void delete(String LeaveId) async {
    try {
      Response response = await post(
        Uri.parse(
            '${ApiConfige.BASE_URL}/parent_delete_child_leave.php'),
        body: {
          'parent_id': Constants.parentId,
          'child_id': widget.childId,
          'leave_id': LeaveId
